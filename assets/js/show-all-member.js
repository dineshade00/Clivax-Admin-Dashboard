$(document).ready(function () {
  ...
  $("#datatable-col-visiblility tbody").on(
    ...
    function () {
      ...
      if (member) {
        $("#profileMemberId").text("#CA-" + String(member.id).padStart(5, "0"));
        $("#profileFullName").text(member.firstName + " " + member.lastName);
        ...
        $("#profileExpiryDate").text(member.expiry_date || "N/A");
  ...
  $("#profileMembershipCardBtn").on("click", function () {
    const memberIdString = $("#profileMemberId").text().replace("#CA-", "");
    const memberId = parseInt(memberIdString);
    const members = JSON.parse(localStorage.getItem("members")) || [];
    const member = members.find((m) => m.id === memberId);

    if (member) {
      $("#cardMemberId").text("#CA-" + String(member.id).padStart(5, "0"));
  ...
  $("#datatable-col-visiblility tbody").on(
    ...
    function () {
      ...
      if (confirm("Are you sure you want to delete " + memberName + "?")) {
        let members = JSON.parse(localStorage.getItem("members")) || [];
        members = members.filter((member) => member.id !== memberIdToDelete);
...
function populateMemberProfile(member) {
  ...
  document.getElementById("profileContact").innerText = member.contact || "";
  ...
  document.getElementById("profilePincode").innerText = member.pincode || "";
  document.getElementById("profileMembershipType").innerText =
    member.membership_type || "";
  document.getElementById("profileStatus").innerText = member.status || "";
  ...
  // ✅ NEW: Show Joining and Expiry Date
  document.getElementById("profileJoiningDate").innerText =
    member.joining_date || "";
  document.getElementById("profileExpiringDate").innerText =
    member.expiry_date || "";
}
